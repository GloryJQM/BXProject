...
@property (nonatomic, assign) CGFloat use_coupon; //使用了多少代金券
...
- (void)viewDidLoad {
    ...
    _freight = [self.patInfoDic[@"freight"] floatValue];
...
- (void)creationScrollView {
    self.use_coupon = [self.patInfoDic[@"use_coupon"] floatValue];
    ...
    NSString *price;
    NSString *total_money = [NSString stringWithFormat:@"%@", self.patInfoDic[@"total_money"]];
    if (self.is_point_type == 1 && _freight > 0) {
        NSString *total_money_coupon = [NSString stringWithFormat:@"%@", self.patInfoDic[@"total_money_coupon"]];
        NSString *freight = [NSString stringWithFormat:@"%@", self.patInfoDic[@"freight"]];
        
        price = [NSString stringWithFormat:@"%@(运费%@)", [total_money_coupon moneyPoint:self.is_point_type], [freight money]];
    }else {
        price = [NSString stringWithFormat:@"%@", [total_money moneyPoint:self.is_point_type]];
    }
    ...
    if (self.use_coupon > 0) {
        ...
        NSString *coupon = [NSString stringWithFormat:@"%@", self.patInfoDic[@"total_money_coupon"]];
        ...
        NSString *money = [NSString stringWithFormat:@"%@", self.patInfoDic[@"total_money"]];
    ...
    NSArray *payment_modeAry = self.patInfoDic[@"payment_mode"];
    
    NSInteger index;
    if (self.is_point_type == 1) {
        index = _freight > 0 ? payment_modeAry.count : 1;
    }else {
    ...
    for (int i = 0; i < index; i++) {
        ...
        if (self.is_point_type == 1 && [dic[@"name"] isEqualToString:@"积分"]) {
            ...
            NSString *points_num = [NSString stringWithFormat:@"%@", self.messageDic[@"user_points"]];
            
            NSString *payString =  [NSString stringWithFormat:@"积分(剩余%@)", [points_num moneyPoint:self.is_point_type]];
            NSMutableAttributedString *attr = [[NSMutableAttributedString alloc] initWithString:payString];
            NSRange range = [payString rangeOfString:[NSString stringWithFormat:@"(剩余%@)", [points_num moneyPoint:self.is_point_type]]];
            ...
            priceLabel.text = [NSString stringWithFormat:@"%@", self.patInfoDic[@"total_money"]];
            ...
            NSString *user_gold = [NSString stringWithFormat:@"%@", self.messageDic[@"user_gold"]];
            NSString *payString =  [NSString stringWithFormat:@"%@(剩余%@金币)", dic[@"name"], user_gold];
            NSMutableAttributedString *attr = [[NSMutableAttributedString alloc] initWithString:payString];
...
- (void)payButton1 {
    ...
    [RCLAFNetworking postWithUrl:@"payOrderApiNew.php" BodyString:bodyStr isPOST:YES success:^(id responseObject) {
