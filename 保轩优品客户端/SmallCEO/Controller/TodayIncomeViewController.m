...
@interface TodayIncomeViewController ()<UITableViewDataSource, UITableViewDelegate, MJRefreshBaseViewDelegate>
{
    UIView *titleView;
}
...
-(void)postRecode
{
    ...
    NSString*body = @"";
    if (self.whereVC == 3) {
         body=[NSString stringWithFormat:@"nowdate=%@&p=%d", dateString,1];
    }else if (self.whereVC == 4){
         body=[NSString stringWithFormat:@"&nowdate=%@&p=%d",date2, 1];
    }
   
    NSString *str=MOBILE_SERVER_URL(@"today_shouyi.php");
...
-(void)postUp
{
    ...
    NSString* date2 = [NSString stringWithFormat:@"%@-%@-%@",stryear,strmonth,strday];
    NSString*body=[NSString stringWithFormat:@"&nowdate=%@&p=%d",date2, self.curPage];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    TodayIncomeCell * cell = [tableView dequeueReusableCellWithIdentifier:cellIndentifier];
    if (!cell) {
        cell = [[TodayIncomeCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIndentifier];
    }
    [cell updateDic:self.dataArray[indexPath.row]];
