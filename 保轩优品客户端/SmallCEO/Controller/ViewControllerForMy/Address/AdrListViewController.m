...
- (void)viewDidLoad {
    ...
    _morenButton.frame = CGRectMake(0, 0, 35, 25);
    [_morenButton setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    [_morenButton addTarget:self action:@selector(gotoEditView) forControlEvents:UIControlEventTouchUpInside];
    UIBarButtonItem * rightItem = [[UIBarButtonItem alloc] initWithCustomView:_morenButton];
    _morenButton.titleLabel.font = [UIFont systemFontOfSize:15];
    self.navigationItem.rightBarButtonItem = rightItem;
    
    
    
    self.tab = [[UITableView alloc] init];
    self.tab.frame = CGRectMake(0, 0, UI_SCREEN_WIDTH, UI_SCREEN_HEIGHT-47-64);
    ...
    UIButton* addAdrBtn = [[UIButton alloc]initWithFrame:CGRectMake(0, UI_SCREEN_HEIGHT-47 - 64, UI_SCREEN_WIDTH, 47)];
...
- (void)handleLeftItemReturn {
    if (self.myAdrList.count == 0) {
        if (self.hdBlock) {
            self.hdBlock(nil, self.curChose);
        }
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    }else
    {
        ...
        if(hdBlock)
        {
            ...
            dics[@"contacts_tel"] = dic[@"phone_num"];
            dics[@"contacts_name"] = dic[@"contact_name"];
            dics[@"contacts_id"] = dic[@"contact_id"];
            hdBlock(dics,self.curChose);
            [self.navigationController popViewControllerAnimated:YES];
        }
...
{
    if (self.myAdrList.count == 0) {
        UIAlertView *alert = [[UIAlertView alloc]initWithTitle:App_Alert_Notice_Title message:@"请先设置一个收货地址" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
        [alert show];
    }else {
        EditAddressListViewController *edit = [EditAddressListViewController new];
        //edit.adrList = self.myAdrList;
        [self.navigationController pushViewController:edit animated:YES];
    }
    
}
#pragma mark - 获取全部地址数据
-(void)postAdrList
{
    [SVProgressHUD show];
...
-(void)goEditAdr:(UIButton*)btn
{
    ...
    vc.myBlock = ^(int current,NSDictionary *dic)
    {
        DLog(@"block 回调");
        _curChose = current;
    };
...
- (void)getAllAddressData
{
    ...
    NSString *str=MOBILE_SERVER_URL(@"getcitylist.php");
    TokenURLRequest *request = [[TokenURLRequest alloc] initWithURL:[NSURL URLWithString:str]];
    [request setHTTPMethod: @"POST"];
    [request setHTTPBody:[body dataUsingEncoding:NSUTF8StringEncoding]];
