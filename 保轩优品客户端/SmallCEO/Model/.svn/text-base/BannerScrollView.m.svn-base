...
-(void)setImageArray:(NSArray *)imageArray {
    ...
    if (_isNetworkImage) {
        ...
        for (NSString *imageName in imageArray) {
            //NSLog(@"imag_userNameeName=%@",imageName);
            NSString *imagePath = [[NSBundle mainBundle] pathForResource:imageName ofType:@"jpg"];
            UIImage *image = [UIImage imageWithContentsOfFile:imagePath];
            
            [localimageArray addObject:image];
            //NSLog(@"local=%@",localimageArray);
        }
...
- (void)setPageIndicatorColor:(UIColor *)pageIndrColor {
    _PageControl.pageIndicatorTintColor = pageIndrColor;
}

- (void)setCurrentPageIndicatorColor:(UIColor *)currentColor {
