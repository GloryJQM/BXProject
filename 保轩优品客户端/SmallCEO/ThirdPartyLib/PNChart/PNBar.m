...
{
    _copyGrade = grade;
    CGFloat startPosY = grade * self.frame.size.width;

    UIBezierPath *progressline = [UIBezierPath bezierPath];

    [progressline moveToPoint:CGPointMake(0, self.frame.size.height / 2.0)];
    [progressline addLineToPoint:CGPointMake(startPosY, self.frame.size.height / 2.0)];
...
-(void)setNumberLabels
{
    ...
    [self.textLayer setFrame:CGRectMake((textWidth-size.width)/2.0,(self.frame.size.height - size.height) / 2.0, size.width,size.height)];
