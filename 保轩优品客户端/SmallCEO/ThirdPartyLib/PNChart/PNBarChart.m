...
- (void)setupDefaultValues
{
    ...
    _labelFont           = [UIFont systemFontOfSize:18.0f];
...
- (void)setYValues:(NSArray *)yValues
{
    ...
    frame.size.height = _barChartHeight * yValues.count + 10 * (yValues.count - 1);
    self.frame = frame;
    _yValues = yValues;
    ...
    if (_showLabel) {
        ...
        for (int index = 0; index < _xLabels.count; index++) {
            ...
            if (labelAddCount == _xLabelSkip) {
                ...
                PNChartLabel * label = [[PNChartLabel alloc] initWithFrame:CGRectMake(0, index * (_barChartHeight + 10), _xLabelWidth, _barChartHeight)];
                label.font = _labelFont;
                label.textColor = _labelTextColor;
                label.layer.borderWidth = 1;
                label.backgroundColor = PNGreen;
...
- (void)updateBar
{
    ...
    for (NSNumber *valueString in _yValues) {
        ...
        if (_bars.count == _yValues.count) {
            ...
            bar = [[PNBar alloc] initWithFrame:CGRectMake(_xLabelWidth, //Bar X position
                                                          index * (_barChartHeight + 10) , //Bar Y position
                                                          self.showLevelLine ? chartCavanHeight/2.0:chartCavanHeight, // Bar witdh
                                                          _barChartHeight)]; //Bar height
            //Change Bar Radius
            bar.barRadius = 0;
