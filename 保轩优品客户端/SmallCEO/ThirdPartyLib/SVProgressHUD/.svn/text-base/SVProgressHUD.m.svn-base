...
@synthesize overlayWindow, hudView, maskType, fadeOutTimer, stringLabel, imageView, spinnerView, visibleKeyboardHeight,cancelBtn;
...
+ (void)showCorrectWithStatus:(NSString *)string {
    [SVProgressHUD show];
    [[SVProgressHUD sharedView] dismissWithStatus:string error:NO afterDelay:1];
}
...
-(UIButton *)cancelBtn
{
    if (cancelBtn == nil) {
        ...
        cancelBtn.frame = CGRectMake(0, 0, 320, 480);
        //[cancelBtn addTarget:self action:@selector(endloaddatas) forControlEvents:UIControlEventTouchUpInside];
        cancelBtn.backgroundColor = [UIColor clearColor];
    }

    if(!cancelBtn.superview)
...
-(void)endloaddatas
{
    UIAlertView *vi = [[UIAlertView alloc] initWithTitle:@"温馨提示" message:@"是否要取消当前请求" delegate:self cancelButtonTitle:@"否" otherButtonTitles:@"是", nil];
    vi.delegate = self;
    [vi show];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex == 1) {
        ...
        //3.
        [self.AFURLOperation cancel];
        [[SVProgressHUD sharedView] dismissWithStatus:@"请求被取消" error:NO];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"cancelreq" object:@"no"];
    }
