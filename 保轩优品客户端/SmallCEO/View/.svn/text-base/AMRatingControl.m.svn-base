...
static const NSInteger kStarWidthAndHeight = 16;
...
- (void)drawRect:(CGRect)rect
{
    ...
    int ratintInt = _rating;
    float decimal = _rating - ratintInt;
    
	for (int i = 0; i < ratintInt; i++)
	{
		if (_solidImage)
        {
    ...
    if (decimal > 0) {
        ...
        CGPathAddRect(path, NULL, CGRectMake(currPoint.x, currPoint.y, kFontSize/2, rect.size.height));
        CGContextAddPath(context, path);
        CGContextClip(context);
        CGPathRelease(path);
        [kDefaultSolidChar drawInRect:CGRectMake(currPoint.x, currPoint.y, kFontSize/2, rect.size.height) withFont:[UIFont boldSystemFontOfSize:kFontSize]];
        CGContextRestoreGState(context);
        
        currPoint.x += kStarWidthAndHeight;
    }
	
	NSInteger remaining = _maxRating - ratintInt - (decimal > 0 ? 1 : 0);
    ...
    CGPoint floatPoint = CGPointZero;
    floatPoint.x += kStarWidthAndHeight*(int)_rating;
