...
- ( NSMutableDictionary *)sendPayWithName:(NSString *)name price:(NSString *)price  orderno:(NSString *)order_no
{
    ...
    //订单标题，展示给用户
    NSString *order_name    = [NSString stringWithFormat:@"%@",name];
    if(order_name.length>=30){
        order_name=[NSMutableString stringWithFormat:@"%@...",[order_name substringToIndex:26]];
    }
    //订单金额,单位（分）
    NSString *order_price   =[NSString stringWithFormat:@"%0.0f",[price floatValue]*100 ];//1分钱测试
    ...
    NSString *orderno   = [NSString stringWithFormat:@"%@",order_no];
    NSMutableDictionary *packageParams = [NSMutableDictionary dictionary];
    NSString *backUrl=SERVER_URL(@"weizhifu/notify_url.php");
    if([name isEqualToString:@"充值"]){
        backUrl=SERVER_URL(@"weizhifu/notify_url_cz.php");
    }
    ...
    [packageParams setObject: backUrl        forKey:@"notify_url"];  //支付结果异步通知
